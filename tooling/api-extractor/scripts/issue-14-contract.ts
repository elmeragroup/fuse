...
export const issue14TimingStableContract = "deterministic-counters-exact" as const;
export const issue14TimingWallClockContract = "observational" as const;
export const issue14TimingWallClockRationale =
  "wall-clock IPC fields vary with scheduler and process load; they are recorded and checked for finite non-negative values, while deterministic counters remain exact and live/stored aggregates stay below 1000ms" as const;
...
export const issue14IpcEvidence =
  "each fixture runs in a fresh public-seam timing session like the Issue 02 baseline; deterministic counters are exact, wall-clock fields are observational, and both live and stored aggregates are checked against 1000ms" as const;
