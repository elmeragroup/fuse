...
Changesets accumulate on `main` across merged PRs. Once publishing is activated, each push publishes a canary and, while changesets are pending, the bot opens or updates the Version Packages PR; merging that PR publishes the stable line. The [release runbook](../docs/spec/release.md) owns the details.
