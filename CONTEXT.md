...
- **Derived role**: a contract token the pipeline computes from other composed roles instead of reading it from a layer, such as `--secondary-hover` (`secondary` mixed 5% toward `foreground`). A theme ships it as a literal, and no palette may assign it.
- **Radius step**: `--radius-step`, the spacing of the `rounded-*` scale around `--radius`. External themes step 2px; the internal variant steps 0px, so every element rounds with one radius.
