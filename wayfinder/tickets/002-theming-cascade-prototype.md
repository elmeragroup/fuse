...
2. Specificity & cascade behavior: attribute selectors vs the class approach, interaction with Tailwind v4 `@theme inline` variable mapping, and with nested re-scoping (OrderModuleWeb re-scopes a `<main>` subtree and portals dialogs _inside_ it so overlays inherit theme vars — the mechanism must support scoped subtrees and portals).
...
1. **Scales without explosion.** 13 rules cover all 16 permutations: `:root` defaults, 5 brand-pointer rules (keyed on brand alone, serving both variants), 1 internal grayscale base, 5 external brand palettes, 1 genuine segment delta (fkas-c). Rules grow with _value differences_, not permutations; missing segment palettes (tkas/guen company) fall back to private **by absence of a rule** — zero fallback CSS.
