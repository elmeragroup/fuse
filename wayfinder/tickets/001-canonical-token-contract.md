...
Internal themes supply the same contract: `--primary` stays neutral (near-black on white), `--brand`/`--sidebar-brand` carry the brand color, `--feature` points at a neutral, everything else inherits library defaults. The variant axis is expressed entirely inside token _values_ — no internal-only names.
