...
`[data-theme-variant="internal"]` overrides **nothing beyond the defaults** except confirming the grayscale identity — in practice the internal rule is nearly empty because layer 1 _is_ the internal look (that was deliberate). All 8 internal permutations = defaults + brand pointer. Segment has **no internal value axis** (confirmed: no internal segment styling exists in the refs).
...
| `--font-heading`              | `"Neo Sans", var(--font-sans)`      | _(inherit)_                         | _(inherit)_                         | _(inherit)_                           |
