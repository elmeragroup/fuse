...
- **M3 roles remapped by evidence, not by name**: `surface→background`, `on-surface→foreground`, `primary-container→card` (the real default card), `surface-bright→card-soft`, `secondary-container→secondary-soft` (and `primary-soft`: P-95, the same tone as `secondary-soft`), `surface-variant family→` a minted `--feature/--feature-bright/--feature-foreground` role (strong brand promo panels).
