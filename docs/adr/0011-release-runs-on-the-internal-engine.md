...
- This repository runs on that engine. `packages/ui` is the one `ReleasePackage`; `scripts/release.ts` resolves it, the [pack adapter](../../packages/ui/scripts/release-pack.ts) owns build → pack → gates (the build stamps the release version and `elmeraRelease` identity into the publish manifest), and the root scripts `release:version`, `release:run`, and `release:check-pr` are the only entry points (release.md §2).
