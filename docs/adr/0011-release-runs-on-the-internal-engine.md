...
- This repository runs on that engine. `packages/ui` is the one `ReleasePackage`; `scripts/release.ts` resolves it, the [pack adapter](../../packages/ui/scripts/release-pack.ts) owns build → stamp → pack → gates (the adapter runs the package build with the release intent so the publish manifest is written once with the release version and `elmeraRelease` identity), and the root scripts `release:version`, `release:run`, and `release:check-pr` are the only entry points (release.md §2).
