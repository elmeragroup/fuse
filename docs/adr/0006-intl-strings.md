...
Date: 2026-08-18. Status: accepted; amended 2026-09-02 — `useLocalizedStrings` caches formatters per dictionary identity and locale; amended 2026-09-03 — dictionaries are assembled by `createStringDictionary`, and a row may be owned by an overlay family rather than one component; amended 2026-09-04 — the factory lives in `intl/`, not `hooks/`.
...
**Assembly is one function.** Each `intl/index.ts` was a byte-identical `new LocalizedStringDictionary({ "en-US": …, "fi-FI": …, "nb-NO": …, "sv-SE": … })`. That assembly is now the package-private `createStringDictionary({ enUS, fiFI, nbNO, svSE })` in `intl/create-string-dictionary.ts`; the four locales are named arguments, so a locale missing from a dictionary is a type error at the call site rather than a runtime lookup miss, and the shared key/value type parameters make the four row modules prove they carry the same keys. Adding a fifth locale stays the mechanical change the decision promised — one argument on the factory plus one module per owner. The factory returns a fresh dictionary per call, so the 2026-09-02 formatter cache is unaffected: identity is still per dictionary object, and one dictionary and locale still yield one formatter instance. A value import of `LocalizedStringDictionary` outside the factory is a lint error (`no-restricted-imports`, with a per-file allow for this module).

**A row's owner may be a family.** §4.1 ownership was read as strictly per component, which forced the same four `close` strings to be authored three times because Dialog, Sheet, and Toast each render the same corner dismiss affordance. The rule is now: a row belongs to the single module where the string is authored, and where several components render the same affordance with the same copy, that family owns it. Exactly one v1 row is family-owned — `overlay.close`, which is the key the §4.1 manifest table now carries in place of the retired `dialog.close` / `sheet.close` / `toast.close` rows. It is authored in `components/overlay/intl/index.ts` (spec 08 named that module `overlay/intl/close`; it ships as the `intl/index.ts` dictionary module §4 mandates for every owner, exporting `overlayCloseStrings`) and read by **four** modules: `dialog.tsx` (Content and Footer), `sheet.tsx`, `toast.tsx`, and the package-private `react-aria/internal/dialog.tsx`, which already read Dialog's dictionary before this amendment. That fourth reader paints its own `dialogVariants().closeButton` rather than the shared `overlayCornerCloseButton`, which is the point: a family-owned row shares the copy, not the markup. Nothing else is shared, and per-component `closeLabel` / `label` props still override at the call site, so no rendered string and no public API changes.

**Status: pending owner confirmation.** The alternative not taken was to keep three per-component `close` rows and merely build them through the factory: fewer moved files, but the same four strings authored in three places, which is the duplication the shared-spine work exists to remove. If the owner rules §4.1 ownership strictly per component, reverting is mechanical — restore the three `intl/` directories, each a `createStringDictionary` call, restore the three manifest rows, and re-point the four reading modules named above (five call sites, since `dialog.tsx` resolves the label on both Content and Footer). The react-aria picker dialog would follow Dialog's row, as it did before.

## Amendment 2026-09-04 — factory lives in `intl/`

`createStringDictionary` is not a hook. It moved from `hooks/create-string-dictionary.ts` to `intl/create-string-dictionary.ts`, beside the locale dictionaries it assembles. `useLocalizedStrings` stays in `hooks/`.
