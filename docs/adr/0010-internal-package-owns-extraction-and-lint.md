...
- A facade that only re-exports a dependency (`focusable`) is walked like any other: since `0.1.1-canary.1` the package publishes each re-export as a part with no props, with `sourcePath` and `rsc` from the authored module and the dependency in `forwardedFrom` ([elmeragroup/internal#4](https://github.com/elmeragroup/internal/issues/4)). The old artifact had `node_modules` source paths instead. A page's own RSC badge is read from its implementation module's directive, never from the artifact.
...
- `@elmeragroup/internal` is a devDependency of the workspace root (for oxlint, which resolves plugin specifiers from the config's directory) and of `apps/docs` (for generation). Both install it through one exact catalog entry; because the line is a canary, that exact version is also named in `minimumReleaseAgeExclude`.
- `tooling/typescript` stays: the shared tsconfig bases are repository configuration, not tooling code.

## Consequences

- `apps/docs` no longer depends on `effect` or on a workspace extractor. `@oxlint/plugins` is not a catalog entry: it is a pinned runtime dependency inside the package and still resolves to the version the catalog used to pin, so the lockfile keeps one copy. `effect` returns to the catalog, pinned to the exact version the engine declares ([ADR 0011](0011-release-runs-on-the-internal-engine.md)). The `minimumReleaseAgeExclude` entry for `effect@4.0.0-rc.115` stays until the package moves to a stable release ([roadmap](../spec/roadmap.md) §12).
