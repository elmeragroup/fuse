...
Normative chapter for `@elmeragroup/ui`: the published package's structure, entry/export surface, build pipeline, JS + CSS distribution, and dependency policy. Sources: Package architecture & build pipeline (wayfinder ticket 008), [ADR 0005](../adr/0005-package-architecture.md), Theme provider API (wayfinder ticket 006), Icon system (wayfinder ticket 009), Spec: date & react-aria interim (wayfinder ticket 024), [ADR 0006](../adr/0006-intl-strings.md).
...
| `@elmeragroup/ui/<component>`                | One bare path for each of the 55 shipped non-interim components (`/button`, `/field`, `/select`, …); `chart` is deferred (Wave 9)                                                                                                                                                                                                                                                                                                                                                                                    | yes             |
| `@elmeragroup/ui/react-aria/<component>`     | Eleven quarantined react-aria interim entries (§2.2)                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | **no**          |
| `@elmeragroup/ui/theme`                      | Server-safe: `themeAttributes`, `defaultDensityForVariant`, `densityAttributes`, `themeSlug`/`parseThemeSlug`, `coerceTheme`, `validateTheme`, `BRANDS`, `BRAND_CODES`, `isBrandCode`, `THEME_VARIANTS`, `THEME_SEGMENTS`, `LEGAL_THEMES`, `ColorSchemeScript`, `colorSchemeScriptSource`, and their public types. Client: `ThemeProvider`, `ThemeScope`, `useTheme`, `useColorScheme`, `ForceColorScheme`, `ElmeraGroupUiProvider`, `useElmeraGroupUi`, and their public types. No per-framework `/theme/*` entries | yes             |
...
- In-repo, the generated `package.json#exports` maps `types` and `import` directly to the TypeScript entries under `src/`; there is no custom `source`/`development` condition. Docs and playground therefore get instant HMR with no package build step and no `transpilePackages` escape hatch needed for publishing correctness. Both Next hosts omit `transpilePackages`; workspace source exports are TypeScript that Next compiles as the app graph. _(Amended 2026-09-02.)_
