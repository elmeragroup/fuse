...
| `@elmeragroup/ui/theme` | `ThemeProvider`, `ThemeScope`, `useTheme`, `themeAttributes`, `themeSlug`/`parseThemeSlug`, `validateTheme`, `BRANDS`, `ColorSchemeScript`, `useColorScheme`, `ElmeraGroupUiProvider`, `useElmeraGroupUi`, and their public types | yes |
...
| `@internationalized/date` | regular | `^3.12.2` | **scoped to the date cluster**: imported only by `react-aria/` date entries; uninstalls from the dependency list together with the cluster at migration ([024](../../wayfinder/tickets/024-spec-date-interim.md)) |
