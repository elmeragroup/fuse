...
| `@elmeragroup/ui/theme`                      | Server-safe: `themeAttributes`, `defaultDensityForVariant`, `densityAttributes`, `themeSlug`/`parseThemeSlug`, `coerceTheme`, `validateTheme`, `BRANDS`, `BRAND_CODES`, `isBrandCode`, `ColorSchemeScript`, `colorSchemeScriptSource`, and their public types. Client: `ThemeProvider`, `ThemeScope`, `useTheme`, `useColorScheme`, `ForceColorScheme`, `ElmeraGroupUiProvider`, `useElmeraGroupUi`, and their public types. No per-framework `/theme/*` entries | yes             |
