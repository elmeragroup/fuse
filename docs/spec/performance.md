...
- **Document-level listeners the library installs** (and no others):
  ...
  5. **Form-reset subscription** (`use-form-reset.ts`, package-private; used by the uncontrolled field composites): one capture-phase `reset` listener per field whose reset it owns, attached to the control's root (`element.current.getRootNode()`), the document in the light DOM or the enclosing shadow root. The listener resolves the control's form association when the event fires, so a control that moves between forms or changes its `form` attribute is followed without a resubscribe and a reset on any other form is ignored. Capture phase means a form that stops propagation during dispatch cannot hide its own reset; `preventDefault()` still suppresses the deferred callback. Cleanup removes the listener when the parent takes ownership (`onReset` is null) or the field unmounts. `use-reset-remount.ts` builds on the same subscription; the hooks are the detailed reference.
