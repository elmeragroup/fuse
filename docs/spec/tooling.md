...
- **`elmera/enforce-variant-standard`** — every component's variant recipe conforms to the tv structure (named recipe, `variants`/`defaultVariants` on recipes that have axes, `VariantProps` typing) from [conventions](components/conventions.md). Axis-less recipes omit those objects.
...
- **Helper-coverage gate:** `src/themed-browser-render.test.ts` and `src/focus-ring-helper.test.ts` run in the `unit` project and walk `src/components/**` and `src/react-aria/**`. They fail on a local re-declaration of a harness helper (`fkasPrivate`, `stampDensity`, `px`, `textboxNamed`, `roleNamed`, `headingNamed`, `cssVarColor`), on any use of the retired `theme-browser-fixtures`, on any test-side `--tw-ring-shadow` outside `test/assert-focus-ring.ts`, on a planted `function roleNamed`, on `setTimeout(` waits in browser suites, and on unsanctioned `querySelector`/`data-slot` locators in the q–z and react-aria suites except spec-§9-cited slot audits. Local copies of `roleNamed`/`headingNamed`/`cssVarColor` fail outright; a remaining a–p worklist is a subset check only until the earlier batch tickets empty it. _(Added 2026-09-03 — [ADR 0008](../adr/0008-tests-assert-behaviour-not-source-spelling.md). Amended 2026-09-04 — ticket 49, 2026-09-04: the no-growth subset list is gone for new copies; a planted `function roleNamed` is a failure.)_
