...
vendor/internal/              # packed @elmeragroup/internal until it is on npm (§2)
```
...
- **Vendored `@elmeragroup/internal`** _(amended 2026-09-07, temporary)_: the root and the `docs` workspace install the packed archive under `vendor/internal/` as a `file:` devDependency instead of a registry version, because the verified `0.1.0-canary.1` build could not be published (npm name cooldown). This is the one permitted non-catalog dependency literal; its provenance, checksum, and removal steps are in [`vendor/internal/README.md`](../../vendor/internal/README.md), and `pnpm test:repo-policy` checks the archive hash, both relative paths, and the lint plugin specifiers. Import specifiers are the ordinary package name, so the switch to a published version touches only the two manifests, the lockfile, and this directory. The package pins `typescript` at the catalog version and carries `effect` and `@oxlint/plugins` as its own runtime dependencies; neither is a catalog entry any more.
...
The actual non-browser merge command is dry-run through Turbo in a durable repo-policy test. Its graph must include the root lint and repo-policy tasks and the library, docs and static-theme gates, exclude browser and packed-consumer tasks, and schedule nothing from the retired tooling packages. A second test pins the vendored `@elmeragroup/internal` archive hash, both `file:` dependency paths, and the lint plugin specifiers (§2).
