...
- **oxlint stays on the minor this repo's gate was validated against** _(amended 2026-09-12: `1.82.0`, via a full green `pnpm ci:checks` with every new React-rule hit adjudicated inline)_: `@elmeragroup/internal@0.1.1-canary.1` still pins `@oxlint/plugins` at `1.78.0` and no internal canary for `1.82.0` exists yet, so the next `@elmeragroup/internal` bump revalidates the catalog minor. An oxlint bump here is paired with a package release, not a local plugin edit (§5.3).
