...
- **Build-artifact tripwires:** a test reading a `dist/` artifact asserts that the artifact exists; it never guards itself with `it.skipIf(!existsSync(…))`. Every vitest task in §3 depends on `@elmeragroup/ui#build`, so the artifact is task-graph-guaranteed and its absence is the regression the test is for — a silent skip only hides it. _(Added 2026-09-03 — [ADR 0008](../adr/0008-tests-assert-behaviour-not-source-spelling.md); spec 07 user story 11.)_
