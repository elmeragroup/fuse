...
- `@shadcn/lint` runs five rules at `error`, no shadcn rule left at `warn`, and the root `lint` script stays `oxlint . --deny-warnings`. Three apply on the library source and both apps; their scopes and allows are in the override list below: `shadcn/no-raw-colors`, `shadcn/no-inline-styles` and `shadcn/no-unknown-classes`.
  Two are docs-app rules on `apps/docs/src/**`: `shadcn/no-restyle` (`allow: ["layout"]` plus five contracts — Collapsible is a headless passthrough apart from Content's height transition, Card/Dialog/Field containers keep layout and spacing, Skeleton keeps size and radius, AvatarRoot keeps its ring groups, ScrollAreaRoot keeps the frame's radius and border) and `shadcn/no-arbitrary-values` (`allow: ["layout", "rounded-[inherit]"]` — `rounded-[inherit]` lets the viewport inherit the frame radius the `ScrollAreaRoot` contract allows on `Root`); the override list below carries both rules' contracts and allows. Docs chrome is not exempt: its off-scale values are snapped to the scale; the surviving one-offs are declared in `apps/docs/src/styles/globals.css` — `@utility docs-chip` (em padding on inline code chips) and `@utility docs-swatch-empty` (the empty-swatch hatch).
...
- **Typed custom properties**: the csstype augmentation in `@elmeragroup/typescript-config` (`tooling/typescript/css-custom-properties.d.ts`) types `` `--*` `` keys once. Plain-DOM sites write literal custom properties with no cast, no SAFETY comment, and no disable. A Base UI site that must merge a `--*` var under a state-callback `style` uses `packages/ui/src/styles/merge-style.ts` and carries the one next-line `shadcn/no-inline-styles` disable, because the rule cannot read through a call. A child tsconfig that overrides `compilerOptions.types` must re-list `@elmeragroup/typescript-config/css-custom-properties`; `pnpm test:repo-policy` enforces it. _(amended 2026-09-18)_
- `ignorePatterns` in `.oxlintrc.json`: `**/dist/**`, `**/coverage/**`, `**/.turbo/**`, `**/.next/**`, `apps/docs/src/generated/**`, `plop-templates/**`, `**/.artifacts/**`, `**/.cache/**`, `.scratch/**`, `**/node_modules/**`, `.ref/**`, `packages/ui/scripts/*.mjs`, and the agent-dot dirs (`.agent/**`, `.agents/**`, `.claude/**`, `.codex/**`, `.continue/**`, `.cursor/**`, `.gemini/**`, `.opencode/**`, `.pi/**`, `.roo/**`, `.windsurf/**`). _(Added 2026-09-04. Amended 2026-09-18: added `.scratch/**`, the orchestration scratch workspace, mirroring `.oxfmtrc.json`.)_
- Overrides, in order, scoped exactly as `.oxlintrc.json`:
  ...
  - `packages/ui/src/**/*.{ts,tsx}` — the same three shadcn rules at `error`, with no allows.
  - `packages/ui/src/react-aria/grid-list/grid-list.tsx` — `shadcn/no-inline-styles` allows `pointerEvents`, which react-aria-components injects inline.
  - `packages/ui/src/icons/**`, `packages/ui/src/illustrations/**` and `packages/ui/src/components/emoji/**` — artwork: `shadcn/no-raw-colors` off and `shadcn/no-inline-styles` allows `maskType`.
...
The actual non-browser merge command is dry-run through Turbo in a durable repo-policy test. Its graph must include the root lint, repo-policy, and release-script type-check tasks and the library, docs and static-theme gates, exclude browser and packed-consumer tasks, and schedule nothing from the retired tooling packages. A second test pins `@elmeragroup/internal` to one exact catalog version, checks the matching release-age exclusion, and checks the lint plugin specifiers (§2). A third test reads every `minimumReleaseAgeExclude` entry: it must be the catalog-pinned `@elmeragroup/internal` or `effect` version, or carry a `Temporary: … ages past the guard on <timestamp>` comment whose timestamp is still in the future (§2). A fourth test walks every `tsconfig*.json` under `apps/*`, `packages/*`, `scripts/` and the root, plus every JSON preset in `tooling/typescript` except `package.json`: a config whose `extends` chain reaches `tooling/typescript/base.json` and that sets `compilerOptions.types` must re-list `@elmeragroup/typescript-config/css-custom-properties` (§4). A fifth test walks `apps`, `packages`, `scripts`, `tooling` and `test`: module basenames `utils`, `helpers`, `common`, `misc` are banned; name the module after what it owns. The one allowlisted path is `packages/ui/src/styles/utils.ts`, because the `@elmeragroup/internal` focus-ring rules pin it by path; rename when the rules take the owner path as a rule option (plan ticket 11). A sixth test walks `apps`, `packages`, `scripts`, `tooling` and `test`: every `oxlint-disable` or `eslint-disable` comment (`-next-line` and `-line` forms included) must carry a space-delimited `--` and a reason of at least 8 non-space characters, and one that does not is reported as file:line and rule. _(amended 2026-09-18)_
