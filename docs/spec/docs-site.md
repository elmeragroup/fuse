...
Normative chapter for the `@elmeragroup/ui` docs site, demo pipeline, playground, and AI-docs surface. Sources: Docs site & playground (wayfinder ticket 012), Docs site design prototype (wayfinder ticket 025), [Component spec conventions](components/conventions.md); the aesthetic source of record is the prototype asset the retired docs-design prototype (direction **A — Manual**, as iterated in place to its final form).
...
- **Styling**: docs chrome and authored docs surfaces use statically discoverable Tailwind v4 utility strings. `apps/docs/src/styles/globals.css` is the only docs-owned stylesheet and is limited to Tailwind/library imports, the Typography plugin (`@plugin "@tailwindcss/typography"`), a CSS-first `@utility prose-docs` theme (docs `--tw-prose-*` colors and the mono font for `code`/`kbd`/`samp`/`pre` only — Typography `prose-sm` spacing and type scale are the source of truth; the `max-w-none` width release sits in the `DocsProse` component recipe with the other prose utilities, not in the CSS utility), `@source`, and docs-local `@theme` tokens. Reusable styling contracts live behind React components with private docs `tv` recipes; callers consume those components, never exported class-name constants or public recipes. Component CSS, `@apply`, and dynamic undetectable utilities are not used. `DemoStage` remains the required selector hook for the generated library density artifact, not a docs-owned component stylesheet. _(Amended 2026-09-03 — this bullet placed `max-w-none` inside the `@utility prose-docs` block; it lives on the `DocsProse` recipe.)_
...
- **Two-source production pipeline** _(amended 2026-09-02 — [ADR 0007](../adr/0007-docs-api-extraction-pipeline.md))_: library-owned rows come from the in-repo TypeScript-checker walk (part discovery, printed types, requiredness, defaults, JSDoc, RSC status). Selected Base UI dependency props come from the Effect API extractor (`tooling/api-extractor`) with `includeExternalTypes: ["@base-ui/react"]`; only documented `@base-ui/react` props already counted as forwarded are merged, using the checker's types. HTML and markdown consumers render those props in a separate **Base UI primitive props** group. React and DOM props remain opaque and are represented by the forwarded-prop summary. A default declared by the library wrapper takes precedence over the dependency's JSDoc default. This hybrid superseded the earlier AST-extraction plan (ruling 74b, 2026-08-24: no `@mui/internal-docs-infra`; one committed `api.json` per page).
- **One extraction seam** _(amended 2026-09-03 — [ADR 0007](../adr/0007-docs-api-extraction-pipeline.md))_: `apps/docs/scripts/lib/api` exposes `extractLibraryApi(context, inventory, problems)`, and it is the only walk of a component's public entry. The generation pass, the in-memory `api.json` regenerator behind the drift check, and the shadow comparison all read part inventory, implementation source, RSC status, destructuring defaults, forwarded counts and accepted props from that one model, so they cannot disagree about what a component's API is. A prop's printed type is resolved per prop a consumer asks about, not for every forwarded React and DOM prop up front. Demo and route validation lives only in `docs-inspection`, which the pass imports rather than copies, so a check added there also runs in the shadow. The Effect adapter merges a prop's declarations as extractor nodes; it never splits a rendered type string, which used to tear union members out of a nested type argument.
- **Effect adapter part mapping** _(amended 2026-09-02)_: the shadow Effect view maps hook exports (`function`) and unexpanded namespace/re-export roots (`intrinsic`/`external`) into API parts so the Effect part list matches the checker walk. A namespace object the extractor falls back to `any` still contributes one part per checker-discovered member. `forwardedCount` is the number of props the part accepts that are neither library-declared nor recipe axes, computed from the part type — not hardcoded to zero. The Effect semantic model omits those forwarded props unless `includeExternalTypes` selects their packages, so the count is taken from the same part type the checker walk uses.
- **Template-literal string unions** _(amended 2026-09-02)_: the Effect extractor renders a union of template-literal members as a union of template-literal strings, in source order, and does not emit `unsupported-type-fallback` for that shape. The PhoneNumberField `autocomplete` prop is the library example: the API table shows the legal values instead of `any`.
...
- **Shadow reviewed differences** _(amended 2026-09-02)_: each entry in the docs API shadow snapshot carries a one-line `reason`. An entry without one fails the shadow suite, naming the entry. The shadow run is a declared `test:shadow` turbo task depending on the docs build; it does not reuse the `ci:checks` leaf name. File writes go through the generator writer (`DocsWriter`), not spies on Node builtins.
