...
Normative chapter for the `@elmeragroup/ui` docs site, demo pipeline, playground, and AI-docs surface.
...
2. **MVP scope (this first slice / PoC).** Next workspace app, base-ui docs route-group shape:
   ...
   - Live pages for **Button** and **ScrollArea** that render the library components (package §10 demos may be reused). The HTML artifact ) remains the layout/aesthetic source — never the wiring reference.
