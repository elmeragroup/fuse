...
Normative chapter for the `@elmeragroup/fuse` docs site, demo pipeline, and AI-docs surface.
...
- Docs chrome consumes the library's **internal Elmera theme in both light and dark**. Its colors, fonts and radii use library tokens; preview brand colors stay inside demo scopes. The **document** theme is fixed at `internal-elma-private` on every route that emits `<html>` and does not follow the demo picker (direction B remains rejected).
- **Styling**: docs chrome and authored docs surfaces use statically discoverable Tailwind v4 utility strings. `apps/docs/src/styles/globals.css` is the only docs-owned stylesheet and is limited to Tailwind/library imports, the Typography plugin (`@plugin "@tailwindcss/typography"`), a CSS-first `@utility prose-docs` theme (`--tw-prose-*` colors mapped to library roles, the heading font, and the library mono font for `code`/`kbd`/`samp`/`pre` — Typography `prose-sm` spacing and type scale are the source of truth; the `max-w-none` width release sits in the `DocsProse` component recipe with the other prose utilities, not in the CSS utility), `@source`, and the docs header-height layout token. Docs-specific color/font palettes and forced light color schemes are forbidden; an off-scale docs-chrome value with no scale neighbour — the chip's em padding, the swatch hatch, the swatch's inline `backgroundColor` — lives in the component that needs it and carries a per-file allow in `.oxlintrc.json` (`shadcn/no-arbitrary-values` for the class values, `shadcn/no-inline-styles` for the swatch color), the `grid-list.tsx` pattern, instead of a docs-chrome utility in this stylesheet. _(amended 2026-09-19)_ Reusable styling contracts live behind React components with private docs `tv` recipes; callers consume those components, never exported class-name constants or public recipes. Docs components use `tv` slot recipes for their class maps; a slot string shared by more than one docs component lives in one shared recipe. Component CSS, `@apply`, and dynamic undetectable utilities are not used. The stage's `data-demo-stage` attribute is the required selector hook for the generated library density artifact, not a docs-owned component stylesheet; docs class strings carry utilities only, never marker class names. _(Amended 2026-09-03 — this bullet placed `max-w-none` inside the `@utility prose-docs` block; it lives on the `DocsProse` recipe. Amended 2026-09-04 — ticket 07, 2026-09-04: docs components use `tv` slot recipes for their class maps.)_ Docs recipes do not restate `font-mono` on `code`, `kbd`, `samp` or `pre`; preflight and `prose-docs` own the family. _(amended 2026-09-18)_
...
1. **Allowed first consumer.** After foundation (`/theme`, tokens, public entries) plus `Button` and `ScrollArea` ship, the first consumer **may be the docs app itself** (`apps/docs`). It consumes the real library via workspace source exports — not the HTML mock, and not a prototype bounded inside `packages/fuse`.
2. **MVP scope (this first slice / PoC).** Next workspace app, base-ui docs route-group shape:
   - `(docs)` — the working three-column shell: theme-aware header (wordmark + preview theme settings menu; **no** command-palette / ⌘K search in this slice), a **docs-local SideNav** (left), content, and a **docs-local QuickNav** (right-column on-page TOC). Both navs are app-local compositions over `@elmeragroup/fuse/scroll-area` (a `<nav>` wrapping a `ScrollArea` viewport and vertical scrollbar). They are **not** the library Sidebar and **must not** import `@elmeragroup/fuse/sidebar`. Library Sidebar is not required for docs chrome. This route is the verified Next App Router first-paint host: `themeAttributes` and `densityAttributes(defaultDensityForVariant(DOCUMENT_THEME.variant))` (`dense`) on `<html>`, `ColorSchemeScript` in `<head>` before SkipNav/shell, one `ThemeProvider` with `theme={DOCUMENT_THEME}` (`internal-elma-private`) and `injectColorSchemeScript={false}`, a token-backed canvas that follows the document scheme alongside internal and external demo scopes, `suppressHydrationWarning` for `data-theme`. Docs does not add density as a fourth theme-matrix axis; comfortable is the built-stylesheet contract plus an isolated static-theme stamp.
...
1. The **wordmark** (left).
2. The **theme settings menu**: an icon-only `Button` with the curated `SlidersHorizontal` icon and accessible name `Theme settings` opens `DropdownMenu`, aligned to its right edge. Labeled radio groups expose Appearance, Variant, Brand, and Segment. Selected radio items use the package's pinned checkmark; unselected items have no marker. Brand options use the shared display names. The 280px menu leaves the trailing column for selection checkmarks. Appearance applies to docs and previews; the other groups write docs-local preview state consumed by demo `ThemeScope`s and leave the document's brand attributes unchanged (§4). Pinned brands disable the illegal segment, explain the restriction, and coerce the selection through `coerceTheme`. Radio choices apply immediately and keep the menu open with focus preserved. Escape returns focus to the trigger; an outside click dismisses the menu. The popup scrolls within a short viewport and shrinks to fit narrow screens. `Reset preview theme` restores `internal-fkas-private` without changing appearance. The Appearance group reads `useColorScheme().colorScheme`, the persisted preference, and calls the library's `setColorScheme`. Its options use `COLOR_SCHEME_LABELS` (`Light` / `Dark` / `System`) with `light` / `dark` / `system` values; the matrix keeps its existing three-way control and labels.
3. **Search, ⌘K** (right). Complete-site contract; **omitted from the docs-app MVP** (§2). The MVP header is wordmark + theme settings menu only.
...
This left column is the **docs-local SideNav** (base-ui docs pattern), not `@elmeragroup/fuse/sidebar`. All navigation lives there, grouped base-ui-style with **muted, normal-case group labels**. The groups and their contents:
...
**Component naming in reader-facing copy** _(amended 2026-09-21)_: a component's page title — the SideNav label, the H1, the ⌘K hit, the `llms.txt` row, the `<title>` and the markdown endpoint's heading — is its name spelled as **title-cased words**: `Alert Dialog`, `Date Range Picker`, `UI Providers`. The exported identifier (`AlertDialog`) belongs to the API surface and appears only where it _is_ the API: the import line, the API-reference part headings and anchors, demo source, and inline `code` in prose. The title is **derived from the slug at generation**, never authored: the slug is the one place a component's name is declared, so a label and the import line beneath it can never name two different things. Demo frame titles are sentence case, authored on the `<Demo>` element.
...
1. **H1 + lede** (the derived title, and the one-paragraph description from the MDX frontmatter).
...
- Chrome follows the internal Elmera document palette; cells keep their selected brand, variant and segment. No hatched illegal cells, no coordinate-table framing (direction C, rejected) — the 20 legal permutations only.
...
- Demos import the library via **workspace source exports** only — the exact specifiers a consumer would write (`@elmeragroup/fuse/...`), resolved to source in the workspace. Never relative imports into package internals, never relative imports of other demos.
