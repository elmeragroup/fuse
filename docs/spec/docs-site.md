...
1. The **wordmark** (left).
2. The **theme controls**: four joined mono-font selects, variant · brand · segment · color scheme. The first three write docs-local preview state consumed by demo `ThemeScope`s and leave the document's brand attributes unchanged (§4). Pinned brands disable the illegal segment option; legality rules per [theming](theming.md). The color-scheme select reads `useColorScheme().colorScheme` — the persisted preference — and calls the library's `setColorScheme`, applying and persisting the document scheme. Its three options render the shared `COLOR_SCHEME_LABELS` (`Light` / `Dark` / `System`) and carry the `light` / `dark` / `system` values; the matrix mirrors the same three-way control with the same labels.
