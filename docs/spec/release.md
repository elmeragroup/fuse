...
The publish workflow publishes only when the pack adapter completes. The adapter runs the package's normal build, stamps the release version and `elmeraRelease` identity into `dist/package.json` through the canonical manifest writer, runs `pnpm pack`, and runs the gate scripts against that exact tarball before returning its bytes; the engine verifies the archive against the recorded intent and uploads those bytes. The table below is the **single exhaustive publish-gate list**; owning chapters define each check but must link here instead of maintaining competing release lists.
