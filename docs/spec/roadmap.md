...
- **Why deferred**: `@elmeragroup/internal` pins `effect` to the prerelease as a runtime dependency ([ADR 0010](../adr/0010-internal-package-owns-extraction-and-lint.md)), and this repository's release scripts import it directly ([ADR 0011](../adr/0011-release-runs-on-the-internal-engine.md)); no stable 4.x exists yet. The repo-wide `minimumReleaseAge: 4320` (72 hours) supply-chain guard cannot admit a prerelease, so the pin buys itself a named exclusion. The list's other entries are the exact versions [tooling](tooling.md) §2 lists; all are per exact version, which is what keeps the exceptions temporary rather than a policy hole. _(Amended 2026-09-18.)_
