...
- **Why deferred**: `@elmeragroup/internal` pins `effect` to the prerelease as a runtime dependency ([ADR 0010](../adr/0010-internal-package-owns-extraction-and-lint.md)); no stable 4.x exists yet. The repo-wide `minimumReleaseAge: 4320` (72 hours) supply-chain guard cannot admit a prerelease, so the pin buys itself a single named exclusion — the **only** entry in that list, and the reason the exception is temporary rather than a policy hole.
