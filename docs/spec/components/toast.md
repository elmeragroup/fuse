...
**Manager `add`/`update` options** — base-ui's `ToastManagerAddOptions` shape (`id`, `title`, `description`, `type`, `timeout`, `priority: "low" | "high"`, `actionProps`, `data`, `onClose`, `onRemove`). Our wrapper narrows `type` to the styled statuses: `"error" | "info" | "success" | "warning" | "loading"` (unset → neutral). Explicit `priority` always wins; otherwise the package-private manager adapter supplies `"high"` when the resulting type is `"error"` and `"low"` for every other type. An update that omits both `type` and `priority` preserves the existing priority; an update that changes `type` and omits `priority` derives the new default. Partial updates preserve every omitted field, including status when only priority changes. Explicit `type: undefined` clears status to neutral and derives low priority unless an explicit priority wins; `priority: undefined` is treated as omitted. Other explicitly undefined fields pass through to Base UI (for example, `description: undefined` clears that copy). Both `Toast.useToastManager()` and `Toast.createToastManager()` expose the adapted methods, so in-tree and module-scope calls cannot drift. `add` returns a `toastId`; re-adding an existing `id` updates in place (base-ui upsert, `updateKey` increments). `close(toastId?)` closes one or — with no id — all toasts.
...
7. **Shared overlay spine adopted**: `Toast.Viewport` composes `OverlayPortal` (theming.md §7.4), `OverlayContainerProps` and `selfFocusRingClass` instead of restating the portal-wait guard, the `container` prop shape and the focus-ring constant; the shared `container` doc sentence replaces Toast's near-identical wording in the published prop table — and, because a toast viewport is not a popup, that shared sentence is reworded overlay-neutral ("Portal target for this overlay. Defaults to the nearest enclosing `ThemeScope` element, so it never escapes the theme that opened it."), which is the published text for all eight consumers of `OverlayContainerProps`. The duplicated `SAFETY` comment on each manager face is reduced to one per cast, and the two `Object.prototype.toString.call` guards in the manager adapter become honest `typeof` checks with named disables: they narrow Toast's own manager unions, not a `ReactNode`, so the shared `isTextNode`/`isTextValueNode` helpers do not apply — an options object is not a `ReactNode`, and no shared guard narrows the `promise()` factory arm. The rendered class set and DOM are unchanged. _(Amended 2026-09-03.)_
8. **Error priority fixed:** unlike base-ui's type-agnostic low default, both public manager faces derive high priority for `type="error"` and low for every other type unless callers override `priority`; promise rejection follows the same rule.

9. **Close copy comes from the shared overlay `close` row**: the toast close button reads `overlayCloseStrings` from `components/overlay/intl` instead of a Toast-local `intl/` directory. The four rendered strings, the explicit `label` override precedence, and the DOM are unchanged (accessibility.md §4.1, amended 2026-09-03). _(Amended 2026-09-03 — ADR [0006](../../adr/0006-intl-strings.md), amendment 2026-09-03; **pending owner confirmation**; the alternative not taken was keeping a Toast-local `close` row built through `createStringDictionary`.)_

Kept faithfully from base-ui: the entire manager method surface (`add`/`update`/`close`/`promise` semantics, upsert by `id`, `updateKey`); F6 viewport hotkey; `data-base-ui-swipe-ignore`; stacking CSS-variable contract; `limit` + `data-limited` inert behavior. 10. **The neutral/loading fill is the shared popup fill** (2026-09-03): `toastVariants`' `neutral` and `loading` arms compose `overlayPopupFillClass` instead of restating `bg-popover text-popover-foreground` twice. Toast takes the **fill only**, never `overlayPopupEdgeClass`: it paints its own `ring-1 ring-border` hairline rather than the popup's elevation-plus-ring pair, and the two constants are separate precisely so a family can take one without the other (popover.md §8.6, sheet.md §8.11 take the same seam the same way). The four status arms that carry a soft status fill are untouched, and the rendered class set is unchanged.
