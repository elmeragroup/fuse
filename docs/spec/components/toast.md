...
7. **Shared overlay spine adopted**: `Toast.Viewport` composes `useResolvedPortalContainer` (theming.md §7.4), `OverlayContainerProps` and `selfFocusRingClass` instead of restating the portal-wait guard, the `container` prop shape and the focus-ring constant; the shared `container` doc sentence replaces Toast's near-identical wording in the published prop table. The duplicated `SAFETY` comment on each manager face is reduced to one per cast. The rendered class set and DOM are unchanged. _(Amended 2026-09-03.)_
