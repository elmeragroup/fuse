...
9. **Package-private `FieldFrame` owns the composite label row** — `field/field-frame.tsx` renders the label row (label, an optional component-owned status face, and the optional pending→success crossfade), the description, an optional wrapper grouping control-with-description, and the error: the shape TextField, NumberField and TextareaField each rebuilt (spec 08, finding S18). It is package-private — absent from `package.json#exports`, no public prop table — and carries no client directive because it owns no state; `source-contracts.test.ts` pins that classification and pins its consumers against growing label/description/error markup again. It renders **one** `Field.Root`, so the nested-root name-shadowing trap in §7 is neither reintroduced nor widened and the card-CSS selectors on `Field.Label` are untouched. Rendered class sets are unchanged for TextField and TextareaField; NumberField's label row gains the crossfade (number-field.md §8.7). PhoneNumberField became the fourth consumer in the same way, class set for class set; it is the only one whose control slot carries a second child, its hidden submit input, because the frame ends at `Field.Error` and gained no slot after it (phone-number-field.md §8.18). _(Added 2026-09-03 with the migration it describes; extended the same day with the PhoneNumberField migration.)_
