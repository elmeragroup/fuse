...
- **Card-style label (nested `Field.Root` inside `Field.Label`, §6):** the nested root shadows the outer one, so the control inside the card registers with it and base-ui points the outer label's `htmlFor` at the outer root's control instead. Pair them explicitly — `Field.Label htmlFor="x"` plus `id="x"` on the control — or use the `Field.Item` row scope (what `CheckboxCard` and `Radio` do). Nesting alone leaves the control unnamed; `field-choice-card.tsx` (§10) is the paired form and the docs browser test asserts the name. _(Amended 2026-09-03 — the wiring caveat was undocumented and the demo hit it.)_
