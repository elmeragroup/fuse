...
No public recipe and **no size axis**. Increment/Decrement share the module-private `stepperButton` `cn("…")` string. The field box pins the `md` rung per [conventions](conventions.md) ruling 2, 2026-08-21: `h-(--control-h-md)` plus `controlInsetMdClass` (the md inset/type pair) on the input, and composes shared `focusRing({ target: "within" })`. The group's chrome is `fieldBoxChromeClass` (§8.8); `numberFieldGroupClass` is a sibling `cn()` composite of that chrome, not a slot of `fieldBox`. Nothing exported from the public entry. _(Amended 2026-09-04 — ticket 06, 2026-09-04: shared group chrome. Amended 2026-09-04: axis-less strings are `cn()`, not identity `tv` wrappers.)_
