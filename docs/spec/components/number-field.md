...
No component-specific tv recipe and **no size axis** — the field box is a single fixed `h-9` styling and composes shared `focusRing({ target: "within" })`; a module-private `stepperButton` class string is shared by Increment/Decrement. Nothing exported.
