...
- **Overlay components take `container?: HTMLElement | RefObject<HTMLElement | null>`** and default to the nearest `ThemeScope` under the lifecycle rules in [theming](../theming.md) §7.4. The ref object is the one React 19 `useRef<HTMLElement>(null)` produces, so `| null` is part of the published type. _(Amended 2026-09-03 — every chapter wrote `RefObject<HTMLElement>`; the shipped overlays all take `RefObject<HTMLElement | null>`.)_
...
- Tests co-located: `*.test.ts` (unit project) / `*.browser.test.tsx` (browser project). **All queries role/label-based**; keyboard-interaction tests cover each spec's §7 behaviors. Written fresh — never ported. Unit tests assert recipe class output and pure functions; they do not read component source to pin spelling. Remaining source-level invariants live in `packages/ui/src/source-contracts.test.ts`, each with the reason it is not a lint rule. _(Amended 2026-09-02 — [ADR 0008](../../adr/0008-tests-assert-behaviour-not-source-spelling.md).)_
- Dual-density browser tests follow Button: stamp `data-density` on `document.documentElement` (`dense` and `comfortable`), assert computed metrics against the signed `--control-*` ladder, and prove nested `data-density` / `ThemeScope` variant changes do not rescope. Ticket 79, 2026-08-22: `pnpm gen component` stubs this dual-density browser-test pattern alongside the docs page. Focus rings use the shared helper in `packages/ui/test/assert-focus-ring.ts` (ring on `:focus-visible`, absent on mouse focus, both density stamps). Role, heading and role-token queries use the shared `roleNamed` / `headingNamed` / `cssVarColor` exports of `packages/ui/test/themed-browser-render.tsx` rather than a per-suite copy; `cssVarColor` needs an element under the `ThemeScope` and a suite that imports `dist/themes.css`. _(Amended 2026-09-03 — [ADR 0008](../../adr/0008-tests-assert-behaviour-not-source-spelling.md); gate in [tooling](../tooling.md) §8.)_
