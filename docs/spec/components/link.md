...
| `className`                                                                                                      | `string \| (values: LinkRenderProps & { defaultClassName }) => string` | —       | RAC's own className type, both forms handled by `composeRenderProps`: the recipe and the focus ring resolve first, the caller's classes merge last |

No `usePredictedEvents`/intent props exist in the ref's surface — it is a plain RAC `Link` wrapper; nothing beyond RAC's own props is added. `className` is the one place the port is wider than the ref: the ref merged a plain string, this port keeps RAC's string-or-render-prop union so `isFocusVisible` can feed the shared focus ring (§8.8).
...
8. **`className` keeps RAC's render-prop union** (2026-09-03): §3 described a plain `string` and claimed the ref does not compose render-prop classNames. `LinkProps` is `ComponentPropsWithoutRef<typeof ReactAriaLink> & VariantProps<typeof linkVariants>`, so `className` is RAC's `string | (values) => string`, and `composeRenderProps` resolves it after the recipe and the `isFocusVisible` focus ring (§8.6). The text is corrected to the shipped type; the type is not narrowed, because the focus ring needs the render-prop form.
