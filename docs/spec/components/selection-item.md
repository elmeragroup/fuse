...
8. **`SelectionGroupFrame` and the one orientation map are package-private here** (2026-09-03): `selection-item.tsx` gains the fieldset skeleton for the labeled selection groups — `Field.Root` (with `name`, `invalid`, `disabled`) → `Field.Set` → legend → description → the group primitive → `Field.Error` — plus `selectionGroupOrientationClass`, the single `group`/`list` orientation map, and `renderSelectionItemCardGroup`, the one card-group body that `CheckboxItemGroup` and `RadioItemGroup` call with their own group as the parameter (checkbox.md §8.11, radio-group.md §8.13; spec 08 finding S18). None of the three is exported from `package.json#exports` or the `SelectionItem` namespace.

   The frame renders **one** `Field.Root`, so the nested-root name-shadowing trap of field.md §7 is neither reintroduced nor widened: a card row's control is still named by its own `Field.Item` label row and the group by its legend. Whether the legend shares a row with a status face is a property of the composite, not of a render, so the frame's props are a union — a composite that opts out of the status row cannot pass a `status` node for the frame to drop. §9 pins the parity of the two families' computed layout, which is what stops the map from being forked back into copies.
