...
6. **Shared overlay spine adopted (tracer)**: `Popover.Content` composes the package-private overlay primitives instead of restating them — `OverlayPortal` for the §7.4 portal-target resolution, `OverlayPositionerProps`/`OverlayContainerProps` for the `align`/`alignOffset`/`side`/`sideOffset`/`container` block, and `overlayPositionerClass` / `overlayTimedPopupClass` / `selfFocusRingClass` for the classes (`overlayTimedPopupClass` is surface + motion + duration; Tooltip still composes motion only, untimed, which is what the fill/edge/motion/duration split is for). The rendered class set, the prop names, their documented defaults, and the DOM are unchanged; Popover is the first consumer of the spine the remaining overlays adopt next. _(ticket 05, 2026-09-04: the three popup parts collapse to `overlayTimedPopupClass`.) (ticket 08, 2026-09-04: `OverlayPortal` owns the wait-not-body rule.)_
