...
6. **Shared overlay spine adopted (tracer)**: `Popover.Content` composes the package-private overlay primitives instead of restating them — `useResolvedPortalContainer` for the §7.4 portal-target resolution, `OverlayPositionerProps`/`OverlayContainerProps` for the `align`/`alignOffset`/`side`/`sideOffset`/`container` block, and `overlayPositionerClass` / `overlayPopupSurfaceClass` / `overlayPopupMotionClass` / `overlayPopupDurationClass` / `selfFocusRingClass` for the classes (the surface and the timing rung are separate constants so Tooltip, which paints neither ring nor `duration-100`, can compose only the parts it wants). The rendered class set, the prop names, their documented defaults, and the DOM are unchanged; Popover is the first consumer of the spine the remaining overlays adopt next. _(Amended 2026-09-03.)_
