...
| `Sidebar.Rail`                      | `button`                                                               | invisible `w-4` (16 px) grab strip on the sidebar edge, painting a 2 px hover line (`after:w-[2px]`, `hover:after:bg-sidebar-border`); `sm:` and up only; click toggles; `tabIndex={-1}` + `aria-hidden`, localized `title`; direction-aware resize cursors _(Amended 2026-09-03 — §8.20: the strip was described as 4 px; `w-4` is 16 px and the 2 px measure is the hover line.)_ |
...
Constrained to the canonical contract's **eight sidebar tokens** : `--sidebar`, `--sidebar-foreground`, `--sidebar-accent`, `--sidebar-accent-foreground`, `--sidebar-border`, `--sidebar-ring`, `--sidebar-brand`, `--sidebar-brand-foreground` — with the **new light values** (`--sidebar: oklch(0.9851 0 0)`; `--sidebar-border` / `--sidebar-accent`: `oklch(0.9219 0 0)`).
...
- **Collapsed icon mode**: `tooltip` on MenuButton restores the hidden label — Tooltip.Content `side="right"`, not rendered unless `state === "collapsed" && !isMobile`. Base-ui Tooltip wires `aria-describedby`. _(Amended 2026-09-04 — §8.22.)_
...
20. **Rail geometry recorded from the class list** (2026-09-03): the Rail is a `w-4` (16 px) strip, `sm:` and up, painting a 2 px hover line (`after:w-[2px]`, `hover:after:bg-sidebar-border`). §2 called it a "4 px grab strip", conflating the strip with the line. Text corrected; the geometry itself is unchanged and stays as the ref shipped it.

21. **Provider callbacks are referentially stable and read committed state** (amended 2026-09-05): `setOpen` and `toggleSidebar` retain one identity for the Provider lifetime. A layout effect publishes `open`, `isMobile` and `onOpenChange` to their callback ref after commit. Suspended or abandoned renders never publish pending values or callbacks to the visible controls. The cmd/ctrl+B listener remains subscribed once.

    Desktop requests in the same synchronous event compose against an event-owned pending value. A boolean followed by an updater, or two inversions, therefore applies in order. A microtask clears that pending value so a later event starts from committed `open`, including when a controlled parent rejects a request without rerendering. The event handler calculates each result, notifies the controlled setter or writes internal state, and writes the cookie once per request. Neither cookies nor notifications execute inside replayable React state-updater functions. Mobile toggles still use the native React functional setter and never write the desktop cookie.

    `SidebarMenuButtonTooltip` remains the only `useSidebar()` consumer in the menu-button path, so a button without a tooltip does not rerender when the rail toggles. Controlled/uncontrolled precedence and emitted DOM stay unchanged.

22. **MenuButton tooltip Content unmounts while expanded** (ticket 52, 2026-09-04): the ref gated the label with `<Tooltip.Content hidden={state !== "collapsed" || isMobile}>`. `hidden` does not stop base-ui from opening the tooltip, and under `Tooltip.Provider delay={0}` the label showed while the sidebar was expanded. `SidebarMenuButtonTooltip` now returns `null` unless `state === "collapsed" && !isMobile`, so the Content is not in the tree at all outside collapsed desktop. The nested `Tooltip.Trigger render={render}` composition is unchanged.

Kept faithfully: all constants (§2) incl. cookie write on **every** `setOpen` (even controlled); the three-branch Root incl. the `collapsible="none"` branch **still emitting `group peer` + `data-state/variant/side`** (deliberate funnel deviation from shadcn — inset wizard dialogs depend on the peer/has selectors); the gap+fixed-container desktop layout with 200ms ease-linear **width** transitions; floating/inset icon-width arithmetic (`+ spacing(4)` gap, `+ 2px` container); mobile Sheet composition (`18rem`, no close button, `sr-only` header); the nested `TooltipTrigger render={render}` composition on MenuButton tooltips (the Content's collapsed-desktop gate is §8.22) _(Amended 2026-09-04 — §8.22.)_; `data-collapsible` only-when-collapsed; MenuAction/MenuBadge size-tracking peers; `SidebarInput`'s `ComponentProps<"input">` typing rationale; `Sidebar.Icon` (funnel addition); RTL `dir` forwarding and the Rail's ltr/rtl cursor logic.
...
- **Concurrency and batching (§8.21)**: a suspended or abandoned controlled close retains the visible open state and its committed callback; toggling requests closed. Two same-event inversions and a boolean followed by an updater compose in order. StrictMode does not duplicate cookie writes or notifications. After rejection, a separate event starts from the visible controlled prop.
- **Callback stability (§8.21)**: across two Trigger toggles the context reports one `setOpen` and one `toggleSidebar` identity; `window.addEventListener("keydown", …)` is called once for the Provider's lifetime and never removed in between; a `Sidebar.MenuButton` whose host element counts its own renders is not re-rendered by a toggle that does move Root's `data-state`.
...
- **Collapsed icon-mode tooltip**: with `tooltip="Orders"` — expanded: hover renders no tooltip node (§8.22); collapsed: hover/focus shows `getByRole("tooltip", { name: "Orders" })` positioned right; the button and tooltip trigger are the same element (nested render); string vs object `tooltip` forms.
