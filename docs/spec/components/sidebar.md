...
21. **Provider callbacks are referentially stable and read committed state** (amended 2026-09-05, review ticket 04): `setOpen` and `toggleSidebar` retain one identity for the Provider lifetime. A layout effect publishes `open`, `isMobile` and `onOpenChange` to their callback ref after commit. Suspended or abandoned renders never publish pending values or callbacks to the visible controls. The cmd/ctrl+B listener remains subscribed once.
