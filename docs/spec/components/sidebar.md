...
21. **Provider callbacks are referentially stable** (ticket 44, 2026-09-03): `setOpen` and `toggleSidebar` read `open`, `isMobile` and the controlled `onOpenChange` through a latest-render ref instead of listing them as `useCallback` dependencies, so both keep one identity for the life of the Provider. The ref is assigned **during render**, not from an effect: the `keydown` handler and the Trigger can fire against a render React has committed but whose effects have not run (and a StrictMode double render re-assigns the same values), so an effect-committed ref would hand cmd/ctrl+B the previous `open`/`isMobile` and toggle from a stale state. Assigning in render is safe here because the ref holds only a mirror of this render's values and nothing reads it during render. The cmd/ctrl+B `keydown` listener is therefore subscribed once rather than re-subscribed on every toggle. The collapsed-rail tooltip moves into a `SidebarMenuButtonTooltip` child, which is now the only `useSidebar()` consumer in the menu-button path — a `Sidebar.MenuButton` without a `tooltip` no longer re-renders when the rail toggles. Setter semantics, cookie writes, controlled/uncontrolled precedence and the emitted DOM are unchanged.
