...
│  └─ CalendarGridBody > CalendarCell (render prop per date, `cellVariants`)
...
`calendarVariants` — slotted tv recipe in `styles/calendar.ts`, **module-private**. Slots: `base`, `header`, `heading`, `headerCell`, `body`, `error` — all invariant, resolved once per render. The day cell is `cellVariants` in the same module: a flattened standalone recipe rather than a slot, because it is the only part with axes of its own and the component resolves it per date. It extends shared `focusRing({ target: "state", isFocusVisible })`. Variant axes on `cellVariants`, driven by RAC cell render props: `isSelected` (false → hover/pressed fills; true → primary fill), `isDisabled`, `isUnavailable`. No public recipe export. Private previous/next Buttons borrow `buttonVariants`, including its self-focus adapter.
...
- `errorMessage` is wired via `Text slot="errorMessage"` → the **invalid selected day's** `aria-describedby`: RAC's `useCalendarBase` hands the error id to `useCalendarCell`, which is where an AT reading the day hears why it cannot be used. The Calendar root is deliberately not described; a caller's own `aria-describedby` passes through to that root untouched.
...
- `errorMessage` renders and is referenced by the invalid selected day's `aria-describedby` when `isInvalid`; a caller's own `aria-describedby` still reaches the RAC root.
