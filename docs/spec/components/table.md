...
- `Table.*`: no public tv recipe, no variant axes — single-look class strings whose in-frame branch is selector-driven (§6), not prop-driven. `TABLE_CELL_CLASSES` is a package-private `cn("…")` string, shared with `VerticalTable.Key`. _(Amended 2026-09-04. Amended 2026-09-04: axis-less string is `cn()`, not a one-slot recipe.)_
