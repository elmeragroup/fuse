...
- Base: `pointer-events-auto fixed bg-popover bg-clip-padding text-sm text-popover-foreground shadow-lg` (no `z-50` — the Popup sits inside the Viewport, which carries the level; §8.6) + per-side positioning (`data-[side=right]:inset-y-0 right-0 h-full w-full border-l`, mirrored for left/top/bottom; top/bottom are `inset-x-0 h-auto` with `border-b`/`border-t`) + the transition/swipe classes below.
