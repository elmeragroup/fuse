...
- Base: the shared overlay popup fill (`bg-popover text-popover-foreground` — Sheet takes the fill only; it paints no ring and its own `shadow-lg`) plus `pointer-events-auto fixed bg-clip-padding text-sm shadow-lg` (no `z-50` — the Popup sits inside the Viewport, which carries the level; §8.6) + per-side positioning (`data-[side=right]:inset-y-0 right-0 h-full w-full border-l`, mirrored for left/top/bottom; top/bottom are `inset-x-0 h-auto` with `border-b`/`border-t`) + the transition/swipe classes below. _(Amended 2026-09-03; §8.11.)_
- Axis `size` (13 values, default `"md"`): same scale as Dialog — `sm…7xl` → `min(var(--container-*),90%)`, `8xl` → `min(1366px,90%)`, `9xl` → `min(1536px,90%)`, `10xl` → `min(1920px,90%)`. Each rung is **one declaration**, `[--overlay-width:…]`, held in the shared overlay module as the `overlaySizeVariants` recipe (composed here via `overlaySizeVariants.variants.size`) and read by both Dialog and Sheet; the two gated consumers `data-[side=left]:sm:max-w-(--overlay-width)` and `data-[side=right]:sm:max-w-(--overlay-width)` live once in the recipe base instead of once per rung. Below the `sm:` breakpoint, and always for `top`/`bottom` sides, the panel is full-width (`w-full`) and `size` is inert. _(Amended 2026-09-04; §8.13 — the used width per rung is unchanged; the emitted class set is not.)_
...
6. **z-index deduped**: the ref stamps `z-50` on Overlay, Viewport, **and** Popup (triple). The Popup stamp is dropped; the two remaining stamps are the shared `overlayLayer` constant on the Backdrop and on the Viewport — Sheet's two outermost portalled siblings, which both need the level because neither contains the other. DOM order stacks Backdrop < Viewport, and the Popup inherits from the Viewport. Flat z-50 strategy (all overlays at one level, DOM-order stacking) is deliberate. `overlayLayer` is still spelled once in the overlay module; the popup recipe's `positioner` slot interpolates it rather than restating it. _(Amended 2026-09-03 — this entry and §4 said one stamp; two ship, one per portalled sibling.)_ _(Amended 2026-09-04.)_
...
11. **Shared overlay spine adopted, and the width axis moves to one CSS variable per rung**: `Sheet.Content` composes `OverlayPortal` (theming.md §7.4), `OverlayContainerProps` (whose shared, overlay-neutral sentence replaces Sheet's near-identical `container` wording in the published prop table; toast.md §8.7), `overlayPopupFillClass` and `selfFocusRingClass`; the local `sheetLayer` alias for `overlayLayer` is gone. Sheet takes the **fill only**, not the composed popup surface — it paints `shadow-lg` and per-side borders rather than the surface's `shadow-md ring-1 ring-foreground/10`, and a ring it never wanted cannot be subtracted later (tailwind-merge treats ring width and ring colour as separate conflict groups). The `size` axis first moved onto a CSS variable per rung so the two gated consumers could live once in the recipe base. That pairing was a Sheet-owned `--sheet-width` table (`overlaySheetWidthClasses`) beside Dialog's `max-w-[min(…)]` table (`overlaySizeClasses`), with a unit test deriving one from the other; both tables, the parity test, `--sheet-width`, and `overlaySheetWidthClasses` are gone — the axis is `overlaySizeVariants` / `--overlay-width`, the same 13-row table Dialog reads (§8.13). The used max-width per rung, per side, per breakpoint is identical, and §9 asserts the resolved width rather than the class spelling. _(Amended 2026-09-03.)_ _(ticket 11, 2026-09-04: the size axis is `overlaySizeVariants` / `--overlay-width`, not a Sheet-owned `--sheet-width` table.)_

12. **Close copy comes from the shared overlay `close` row**: `Sheet.Content`'s built-in close button reads `overlayCloseStrings` from `components/overlay/intl` instead of a Sheet-local `intl/` directory. The four rendered strings, the `closeLabel` override precedence, and the DOM are unchanged (accessibility.md §4.1, amended 2026-09-03). _(Amended 2026-09-03 — ADR [0006](../../adr/0006-intl-strings.md), amendment 2026-09-03; **pending owner confirmation**; the alternative not taken was keeping a Sheet-local `close` row built through `createStringDictionary`.)_
13. **One overlay width table**: the `size` axis is `overlaySizeVariants` / `--overlay-width`, the same 13-row table Dialog reads; `--sheet-width` and the Dialog-side `max-w-[min(…)]` table are gone. The two gated consumers are `data-[side=left]:sm:max-w-(--overlay-width)` and `data-[side=right]:sm:max-w-(--overlay-width)`. Option A over B (put `max-w-*` on the popup plus `max-sm:max-w-none data-[side=top]:max-w-none data-[side=bottom]:max-w-none`) because the used width per rung, per side, per breakpoint is already identical through the variable, and collapsing Dialog onto it deletes the parity test without restating Sheet's gates as negations. _(Added 2026-09-04. Amended 2026-09-04: the table is the `overlaySizeVariants` recipe.)_
