...
**Density exemption — optical track.** Switch `default`/`sm` dimensions (`h-[18.4px] w-[32px]`, `h-[14px] w-[24px]`) are ref-verbatim optical values, not the four-rung control box. They stay pixel-fixed at both density stamps and are **not** retokenized onto `--control-h-*`. `elmera/no-hardcoded-density-metrics` leaves arbitrary `h-[…px]` tracks quiet. Dual-density tests assert track/thumb sizes are identical across stamps.

**Kept-but-flagged**: the magic root dimensions are kept for pixel fidelity. Likewise the per-size checked/unchecked thumb translate classes are duplicated verbatim per size even though both resolve to the identical `calc(100%-2px)` / `translate-x-0` — kept as-is (collapsing them changes selector specificity ordering), flagged as a candidate cleanup.
...
3. **Density exemption (optical track):** `18.4px`/`32px` (default) and `14px`/`24px` (sm) root sizes are not the control-box ladder; retained verbatim for fidelity and must not retarget with `data-density` (see §4).
...
- Dual-density exemption: at document `dense` and `comfortable`, track and thumb computed sizes are identical; nested `data-density` does not change them.
