...
| `itemTitleVariants` (private)  | —         | no axes; `title` slot is the Item.Title / Alert.Title face, resolved once as `ITEM_TITLE_CLASSES`                                                                                 | —         |
...
6. `itemVariants` publicity confirmed (ref exports it; `selection-item` borrows it). `itemMediaVariants`/`itemFooterVariants`/`itemTitleVariants` stay private. `ITEM_TITLE_CLASSES` is the title slot, resolved once and re-exported from `item-title-classes.ts` for Alert. _(Amended 2026-09-04.)_
