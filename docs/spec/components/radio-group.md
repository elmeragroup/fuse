...
| `isPending`                     | `boolean`                    | —            | spinner (`SpinnerGap`, `size-3 animate-spin`) at the header row's end; sets `aria-busy` on the radiogroup while true; header renders when `label` or `isPending` is truthy; an unlabeled pending group renders that status row with no legend (§8.2) |
| `orientation`                   | `"vertical" \| "horizontal"` | `"vertical"` | vertical: primitive `flex-col gap-2`, collapsing to `gap-0` over direct shell children (selection-item.md §8.9); horizontal: primitive `flex-wrap gap-4`. On `RadioItemGroup` the same axis also lays out the stacked-card list (§2)                 |
...
- `orientation` is not a local axis: it reads the package-private `selectionGroupOrientationVariants` slotted recipe (`group` + `list` slots, `orientation` axis, `vertical` default) in `selection-item-variants.ts`. `RadioItemGroup` applies that same axis to the stacked-card list (not merely the outer primitive around one child). `radioIconButtonVariants` is the module-private size recipe for `RadioIconButton` (`size` axis, `icon` default) in `radio-group-variants.ts`; it is not exported from `@elmeragroup/ui/radio-group`. `RadioItem` inherits `itemVariants` (outline) through the shell.
...
- `RadioItemGroup` renders the private SelectionItem list (`role="list"`). Vertical (default) is connected `flex-col gap-0`; horizontal is `flex-row flex-wrap gap-4`. `RadioItem` shells adopt `role="listitem"` only inside that group; they stay direct DOM siblings of the list. Vertical/default shells keep connected first/last rounding, `not-first:border-t-0`, and checked `-mt-px`; horizontal groups, card list or plain, render individually rounded full-border cards with neither vertical border collapse nor checked negative margin. Shells placed directly in a plain vertical `RadioGroup` render the same connected `gap-0` stack without list semantics (selection-item.md §8.9).
...
2. **Header-row gate BUGFIX**: ref renders the legend row when `label || isPending !== undefined` — so `isPending={false}` (any boolean wiring) renders an empty legend row with an empty `FieldLegend`. Fixed to gate on truthy `isPending`: row renders when `label || isPending`. _(Amended 2026-09-04: an unlabeled pending group still renders the status row for the spinner, but the legend element is omitted — an empty legend was never a feature.)_
...
13. **Group skeleton and orientation map are shared, not copied** (2026-09-03): `RadioGroup` renders the package-private `SelectionGroupFrame` (selection-item.md §8.8), passing its pending spinner as that frame's `status` face, and reads the shared `selectionGroupOrientationClass` map that `CheckboxGroup` also reads (checkbox.md §8.11; spec 08 finding S18). The heading row renders when `label` or `status` is present, matching FieldFrame; the legend element is omitted when there is no label, so an unlabeled pending group is a status row with no legend (§8.2, 2026-09-04). `RadioItemGroup` inlines the two-line stacked-card wrap (`RadioGroup` around `SelectionItemGroup`); its public signature and `orientation` default (read from its own destructuring) are unchanged. The redundant `errorMessage ? … : null` guard is gone — `Field.Error` already returns null for falsy children. Rendered class sets are unchanged part for part, at both orientations and in the pending, disabled, read-only, required and invalid states. _(Amended 2026-09-04: `SelectionGroupFrame` is gone; `RadioGroup` renders `FieldFrame heading="legend"` from field.md §8.9, still passing the pending spinner as `status`.)_ _(Amended 2026-09-04: shells inside a plain `CheckboxGroup`/`RadioGroup` follow that group's `orientation` too, and the vertical group primitive collapses its gap to `0` over shell children — §8.9.)_ _(Amended 2026-09-04: the orientation map is the package-private `selectionGroupOrientationVariants` slotted recipe in `selection-item-variants.ts`; `RadioIconButton` sizes are the module-private `radioIconButtonVariants` recipe in `radio-group-variants.ts` (`size` axis, `icon` default), not a raw `iconButtonSizes` map.)_
...
- Header-row gate pinned: `isPending={false}` with no `label` renders **no** legend row (bugfix assertion); `isPending` true with a label shows the spinner alongside the label and sets `aria-busy` on the named radiogroup; `isPending` true with no label renders a status row with **no** legend element; `isPending` false or absent omits `aria-busy`.
