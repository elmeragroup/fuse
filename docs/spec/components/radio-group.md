...
13. **Group skeleton and orientation map are shared, not copied** (2026-09-03): `RadioGroup` renders the package-private `SelectionGroupFrame` (selection-item.md §8.8), passing its pending spinner as that frame's `status` face, and reads the shared `selectionGroupOrientationClass` map that `CheckboxGroup` also reads (checkbox.md §8.11; spec 08 finding S18). The §8.2 header-row gate is unchanged and is now the frame's: the row renders when `label || status`, and the legend renders inside it whenever the row does, so a pending unlabeled group still emits the empty legend that gate produces. `RadioItemGroup`'s body is the shared `renderSelectionItemCardGroup(RadioGroup, props)`; its public signature is unchanged. The redundant `errorMessage ? … : null` guard is gone — `Field.Error` already returns null for falsy children. Rendered class sets are unchanged part for part, at both orientations and in the pending, disabled, read-only, required and invalid states.
