...
- `size`: the dialog popup's computed `--overlay-width` matches the axis sample (`sm`, `md`, `10xl`) and `max-width` is set; the `--overlay-width` class strings live in the overlay unit recipe test. _(Amended 2026-09-04; §8.12. Amended 2026-09-04 — spec 07 / [ADR 0008](../../adr/0008-tests-assert-behaviour-not-source-spelling.md): browser asserts the computed custom property, not the class string.)_
