...
- A color becomes an sRGB color, with alpha kept. The tokens write `oklch()` or `#rrggbb` literals, and `@elmeragroup/color` parses them, in the forms its README lists, and clips a color outside the sRGB gamut. A value the sync cannot read fails the run with the reader's own message, such as the color parser's `Expected an oklch(), lab(), rgb() or hex color, received "…"`.
