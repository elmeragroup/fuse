...
function scratchPackage(files: Record<string, string>): string {
  const packageRoot = mkdtempSync(join(tmpdir(), "fuse-barrel-"));
  scratchDirs.push(packageRoot);
  write(packageRoot, "src/styles/fuse.css", "/* fixture */\n");
...
describe("source-export generation ownership", () => {
  ...
  if (parsed === null || Array.isArray(parsed)) {
    throw new Error("packages/fuse/package.json is not an object");
  }
