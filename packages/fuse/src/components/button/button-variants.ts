...
// recipe, and `data-disabled:` covers Base UI and React Aria roots that render another
// element, such as `render={<a />}`, which never matches `:disabled`.
// Every size rounds with the theme's `--radius-button`. The arbitrary-variable form keeps
// tailwind-merge able to replace it with a consumer `rounded-*` class, which the custom
// `rounded-button` utility would not be. Inside a ButtonGroup a button takes the group's
// `rounded-md` silhouette instead, so it lines up with the inputs and text it joins.
export const buttonVariants = tv({
  base: cn(
    "group/button font-medium box-border inline-flex shrink-0 items-center justify-center rounded-(--radius-button) border border-transparent bg-clip-padding p-0 whitespace-nowrap transition-[color,background-color,border-color,box-shadow,translate,opacity] select-none active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 in-data-[slot=button-group]:rounded-md aria-invalid:border-error aria-invalid:ring-3 aria-invalid:ring-error/20 data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
    selfFocusRingClass
  ),
  variants: {
    variant: {
