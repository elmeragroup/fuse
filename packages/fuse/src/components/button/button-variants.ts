...
// Every size rounds with the theme's `--radius-button`. An internal theme leaves it unset,
// so the fallback reads `--radius` on the button itself and follows a subtree override.
// The arbitrary value keeps tailwind-merge able to replace it with a consumer `rounded-*`
// class, which the custom `rounded-button` utility would not be.
...
export const buttonVariants = tv({
  base: cn(
    "group/button font-medium box-border inline-flex shrink-0 items-center justify-center rounded-[var(--radius-button,var(--radius))] border border-transparent bg-clip-padding p-0 whitespace-nowrap transition-[color,background-color,border-color,box-shadow,translate,opacity] select-none active:not-aria-[haspopup]:translate-y-px disabled:pointer-events-none disabled:opacity-50 in-data-[slot=button-group]:rounded-md aria-invalid:border-error aria-invalid:ring-3 aria-invalid:ring-error/20 data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
    selfFocusRingClass
  ),
  variants: {
    variant: {
