...
The `lab()` parser and the CIE Lab conversion live in `src/lab.ts` without a subpath, because callers only meet `lab()` as a computed color that `css-color` reads. The errors `InvalidColor` and `OutOfRange` have no subpath either. A caller meets them in a result and reads their `_tag`, `message` and fields there. The `ok` and `err` constructors stay internal, so only the package builds a result.
