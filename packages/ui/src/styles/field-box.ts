 ...
 *
 * Sibling recipes, not slots of {@link fieldBox}: that recipe is a flat `box` axis
 * consumed as a string by Input/Textarea, and chrome is also painted by RAC
 * `fieldGroupVariants` and Select's trigger, which are not field-box variants.
 * NumberField's group is a second sibling — same chrome, plus flex/`within` focus/
 * invalid ring. Export names stay; both values are resolved once from the recipes.
 *
 ...
 */
const fieldBoxChrome = tv({
  base: "shadow-xs rounded-md border border-input bg-card transition-[color,border-color,box-shadow]",
});

export const fieldBoxChromeClass = fieldBoxChrome();
...
export const numberFieldGroupClass = numberFieldGroup();
