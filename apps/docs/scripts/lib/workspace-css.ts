...
/**
 * Workspace CSS files docs `@import`s that only exist after the UI package
...
export function assertDocsUiCssExports(packageRoot: string): string[] {
  return DOCS_UI_DIST_CSS.map(({ exportKey, relative }) => {
    const absolute = path.join(packageRoot, relative);
    if (!existsSync(absolute)) {
      throw new Error(
        `${exportKey} is exported from ${packageRoot} as ./${relative}, but that file is missing. Run: pnpm --filter @elmeragroup/ui build`
      );
    }
    return absolute;
  });
